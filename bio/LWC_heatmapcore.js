function heatmap(selector, data, options) 
{
    ...
    console.log(data.dend_col)
    dend_row_newick_format = "(((1,2),(3,4)),5)";
  ...
  function colormap(svg, data, width, height) 
  {
    ...
    for(var i =0; i < cluster.length; i++ )
    {
      ...
      {
        cluster_change_rows.push({ylocation:i, cluster:current_cluster_value}); //Cluster changes its value at this Y-location.
      ...
      selection
          ...
          .attr("y", function(d, i) 
          {
            ...
            for(var j =0; j < cluster_change_rows.length; j++)
            {
              ...
              {
                        location_object_array.push({begin:null,end:y(Math.floor(i / cols)), cluster:cluster_change_rows[j].cluster});
  ...
  {
  	console.log("Entered Axis-Label Function");
    svg = svg.append('g');
...
function drawLines(table, links1)
{
  ...
  {
  	console.log("Entered DendoGram Function");
...
///////////////////// ****************** Some New experimental TESTING CODE ****
  ...
  location_object_array.push({begin:null, end: height,cluster:cluster_array[cluster_array.length-1]}); // add the information for the last array
...
debugger;
    ...
    function draw(selection) 
    {
      console.log("Entered DendoGram Draw Function");
