function heatmap(selector, data, options) 
{
  ...
  console.log("Entered heatmapCore");

  ///////////////////////////////////////////////////////////////// new code ////////////////////////////////////
  //Determining what is the column Index value for list.
  var columns = data.matrix.cols.length;
  var list_column_index = -1;
  for(var i =0;i < data.matrix.cols.length;i++)
  {
    if(data.matrix.cols[i] == "list")
    {
      list_column_index = i;
      console.log("Column index for list: " +  i);
    }
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  (function() 
  {
    console.log("Entered unnamed function 1");
  ...
  (function() 
  {
    console.log("Entered unnamed function 2");
  ...
  function colormap(svg, data, width, height) 
  {
    ...
    svg = svg
        .attr("width", width)
        .attr("height", height);
    var rect = svg.selectAll("rect").data(merged);   // EACH RECTANGLE IS DEFINED HERE.
...
/////////////////////////////////////////////////// new code //////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Now determine on which rows the cluster change.
var cluster_change_rows =[];
var rect2 = svg.selectAll("rect").data();

// Check for when the value of List change.
var current_cluster_value = rect2[list_column_index].label; 
for(var k =0; k < rect2.length; k = k + columns)
{
  //console.log("Column "+rect2[k+1].label);
  if(rect2[k+list_column_index].label != current_cluster_value)
  {
    console.log("Cluster Changes at this row HERE !");
    console.log(rect[0][k].rowIndex);
    cluster_change_rows.push(rect[0][k].rowIndex);
    current_cluster_value = rect2[k+list_column_index].label;
  }
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
      console.log("Entered the Draw Lines Function");
      array = [42, 72, 105, 122];
            console.log("Removing Lines");
      d3.selectAll("path").remove();

                // Calculating the index value for the column List.
          console.log(data);
            var columns = data.cols.length;
            var list_column_index = -1;
        ...
        //Calculate where the cluster changes.
        
        var cluster_change_rows =[];
        var rect2 = svg.selectAll("rect").data();
        // Check for when the value of List change.
        var current_cluster_value = rect2[list_column_index].label;
        for(var k =0; k < rect2.length-10; k = k + columns)
        {
          //console.log("Column "+rect2[k+1].label);
          if(rect2[k+list_column_index].label != current_cluster_value)
          {
            cluster_change_rows.push(selection[0][k].rowIndex);
            console.log("And at this Y Coordinates");
            current_cluster_value = rect2[k+list_column_index].label;
          }
        }


        array = cluster_change_rows;
      // DRAW LINES HERE !
      console.log(array);
      for(var i =0; i< array.length;i++)
      {
        
        for(var o =0; o <selection[0].length; o++)
        {
          if(selection[0][o].rowIndex == array[i])
          {
                      svg.append("path")
                      .attr("d", lineFunction([[0,selection[0][o].y.animVal.value], [selection[0][o].width.animVal.value*5,selection[0][o].y.animVal.value]]))
                      .attr("stroke","black")
    ...
    function draw(selection) //HERE WE DRAW THE RECTANGLES !!!
    {
          ...
          // Calculate where the data changes dor the first time.
        var cluster_change_rows =[];
        var rect2 = svg.selectAll("rect").data();
        // Check for when the value of List change.
        var current_cluster_value = rect2[list_column_index].label;
      ...
      selection2 = selection // Here the new selection is calculated.
          .attr("x", function(d, i) // X-Coordinate of each rectangle. 
          {
            
            return x(i % cols);
          })
          .attr("y", function(d, i) // Y-Coordinate of each rectangle.
          {
            // New Code.
            for(var j =0; j <array.length; j++)
            {

            if(selection[0][i].rowIndex == array[j])
            {
                      
                      svg.append("line")
                      .attr("x1", 0)
                      .attr("y1", y(Math.floor(i / cols)))
                      .attr("x2", selection[0][i].width.animVal.value*5)
    ...
    draw(rect,cluster_change_rows);



    controller.on('transform.colormap', function(_) 
    {
      console.log("Gonna Transform the Colormap");
      x.range([_.translate[0], width * _.scale[0] + _.translate[0]]);
      y.range([_.translate[1], height * _.scale[1] + _.translate[1]]);
      draw(rect.transition().duration(opts.anim_duration).ease("linear")); // This is the old selection
    });
  ...
  function axisLabels(svg, data, rotated, width, height, padding) {
    ...
    {
      console.log("LAY OUT MOUSE TARGETS");
      selection
  ...
  function dendrogram(svg, data, rotated, width, height, padding) 
  {
    ...
    console.log("Going to enter the draw for dendograms");
    draw(lines);
  }
