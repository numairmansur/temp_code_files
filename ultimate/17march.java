...
public class FlowSensitiveFaultLocalizer {
	...
	public FlowSensitiveFaultLocalizer(IRun<CodeBlock, IPredicate> counterexample,
		...
		//ArrayList<int[]> informationFromCFG = computeInformationFromCFG( (NestedRun<CodeBlock, IPredicate>) counterexample, cfg); //Get branch information. in the form of an array list
		computeFlowSensitiveTraceFormula(errorPrecondition, counterexample,(NestedWord<CodeBlock>) counterexample.getWord(), predicateUnifier.getFalsePredicate(), modGlobVarManager, smtManager,informationFromCFG);
	}
	
	private ArrayList<CodeBlock> ComputeRelevantStatementsInBranch()
	{
	...
	private void computeFlowSensitiveTraceFormula(IPredicate errorPrecondition, IRun<CodeBlock, IPredicate> counterexampleRun, NestedWord<CodeBlock> counterexample,
	...
	
	{
		...
		for(int i = backward_counter-2 ; i >= 0; i--)
		{
			...
			int pre_counter = wp_counter + 1;
				...
				ERelevanceStatus relevance = rc.relevanceInternal(pre, statement, smtManager.newPredicate(smtManager.not(wp)));
		...
		// Step 2:
		// come up withh the markhor formula of the branch.
		...
		while(backward_counter != -1)
		{
			...
			{			
				...
				TransFormula markhor = TransFormula.computeMarkhorTransFormula(combined_transition_formula, smtManager.getBoogie2Smt(), 
						m_Services, m_Logger);
				//Iterate backward inside the branch here.
				for(int j= b; j>a; j--) 
				{
					weakest_precondition_2 = weakest_precondition_1;
					weakest_precondition_1 = pt.weakestPrecondition(weakest_precondition_1,counterexampleWord.getSymbolAt(j).getTransitionFormula());
					pre_precondition_1 = smtManager.newPredicate(smtManager.not(weakest_precondition_1));
					statement_1= counterexampleWord.getSymbolAt(j);
					relevance = rc.relevanceInternal(pre_precondition_1, statement_1, smtManager.newPredicate(smtManager.not(weakest_precondition_2)));
					if(relevance.toString() == "InUnsatCore")
					{
						use_markhor=1;
						m_Logger.warn("RELEVANT");
						m_Logger.warn(counterexampleWord.getSymbolAt(j));
					}
				...
				if(use_markhor == 1)
				{
					weakest_precondition_1 = pt.weakestPrecondition(weakest_precondition_1_temp, markhor);
				}
		...
		///////////////////////////////    INCREMENTAL flow sensitive FAULT LOCALIZATION    ////////////////////////////////////////////////////
